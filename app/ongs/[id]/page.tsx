  ...
  useEffect(() => {
    const fetchOngData = async () => {
      try {
        ...
        try {
          const eventsResponse = await api.events.getAll({ limit: 100 });
          const filteredEvents = eventsResponse.data.filter(event => event.ongId === params.id);
          setOngEvents(filteredEvents);
        } catch (eventsError) {
          console.error('Error fetching events:', eventsError);
        }
      } catch (err) {
        console.error('Error fetching ONG:', err);
        setError('ONG non trouvée');
      } finally {
        setLoading(false);
      }
    };
